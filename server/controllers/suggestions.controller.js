...
export const foodSuggestion = async (req, res) => {
  try {
    ...
    const suggested_food_id = await sql`SELECT food_id FROM mood_food_mapping WHERE mood_id= ${mood_id}`;
    //WILL RETURN ARRAY VALUE [{food_id: 1}, {food_id: 2}, {food_id: 3}] SO,

    // Check if any foods found for this mood
    if (suggested_food_id.length === 0) {
      return res.status(404).json({ message: "No foods found for this mood" });
    }
